<template>
    <div>
        <el-container style="height: 100%;" class="bgimg">
            ...
            <el-container>
                <el-aside width="230px" style="background-color:#4f7dbd79; height: 100%;">
                ...
                <el-main style="background-color: #4f7dbd92;">
                    ...
                    <div style="margin-top: 5px; text-align: center;">
                        <el-input placeholder="请输入内容" v-model="queryOrderContent" style=" width: 50%;">
                            <el-select v-model="selectCondition" slot="prepend" placeholder="请选择"
                                ...
                                <el-option label="客户名" value="gname"></el-option>
                            </el-select>
                            <!-- <el-form-item label="下单日期">
                                <el-date-picker v-model="orderPeriod" type="daterange" range-separator="至"
                    ...
                    <el-table :data="orderData" style="margin-top: 20px; color:#222;"
                        ...
                        <el-table-column prop="orderday" label="下单日期">
                        </el-table-column>
                        ...
                        <el-table-column prop="operation" label="操作">
                            <template slot-scope="scope">
                                ...
                                <el-button @click.native.prevent="completeOrder(scope.$index, orderData)" type="text"
                                    size="small">
                                    设置完成
                                </el-button>
                            </template>
...
export default {
    ...
    methods:{
        onSubmit: function () {
            // alert(this.queryOrderContent);
            if (this.selectCondition == "ordernum")
            {
                axios.post("http://localhost:8080/orderManagement", {
                    ordernum: this.queryOrderContent,
                    forWhat: "orderManagement",
                    page:"1"
                }
                ).then(result => {
            ...
            if (this.selectCondition == "gname")
            {
                axios.post("http://localhost:8080/orderManagement", {
                    gname: this.queryOrderContent,
                    forWhat: "orderManagement",
                    page: "1"
                }
                ).then(result => {
        ...
        handleCurrentChange: function (val) {
            ...
            else if (this.selectCondition == "status") {
                axios.post("http://localhost:8080/orderManagement", {
                    status: this.queryOrderContent,
                    page: val,
                    forWhat: "orderManagement"
            ...
            else if (this.selectCondition == "gname") {
                axios.post("http://localhost:8080/orderManagement", {
                    gname: this.queryOrderContent,
                    page: val,
                    forWhat: "orderManagement"
        ...
        deleteRow(index, rows) {
            ...
            axios.get("http://localhost:8080/deleteFromOrder", {
                params: {
                    ordernum: rows[index].ordernum
                }
            });
            rows.splice(index, 1);

        },
        completeOrder(index, rows) {
            //这行代码利用 JavaScript 的 splice 方法从 rows 数组中删除元素。
            //splice 方法的第一个参数是开始的索引，第二个参数是要删除的元素的数量。

            axios.get("http://localhost:8080/completeOrder", {
                params: {
                    ordernum: rows[index].ordernum
                }
            });
            //怎么解决必须刷新才能显示的问题？
            //一开始设置成“已完成”了，scope无法识别，导致没有更新
            rows[index].status = "1";
        ...
        submitItem(status) {
            ...
            axios.post("http://localhost:8080/changeCommodity", formData).then(() => { alert("操作成功！") })
            this.pictureFile = {};
            this.itemInfo = {};
            this.$refs.upload.clearFiles();
        },
    ...
    mounted() {
        ...
        // this.searchForm['forWhat'] = 'orderManagement';
        axios.post("http://localhost:8080/orderManagement", {
            page: "1",
            forWhat: "orderManagement"
        }).then(result => {
        ...
        axios.get("http://localhost:8080/username").then((result) => this.userName = result.data.userName);
